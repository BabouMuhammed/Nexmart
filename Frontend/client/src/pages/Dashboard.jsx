...
import { getUserOrders, getMe } from '../services/api';
...
export default function Dashboard() {
  ...
  const { wishlist } = useWishlist(); // real wishlist, populated with product data
      ...
      <section className="py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <DashboardCard title="Wishlist Items" value={wishlist.length} icon={Heart} accent="purple" />
          ...
          <motion.div key={activeTab} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            {activeTab === 'wishlist' && (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {wishlist.length === 0 ? (
                  <p className="text-[#A0AEC0] col-span-full">
